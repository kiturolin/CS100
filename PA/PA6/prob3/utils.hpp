...
std::ostream &
operator<< (std::ostream &os, TextColor color)
{
...
struct Starter {
  Starter ()
  {
    ...
    if (hStd == INVALID_HANDLE_VALUE) { exit (EXIT_FAILURE); }
    DWORD dwMode = 0;
    if (!GetConsoleMode (hStd, &dwMode)) { exit (EXIT_FAILURE); }
