...
GameWorld::InitializeSeedPacks ()
{
  ...
  auto peashooter_seed = std::make_shared<Seed> (
      ImageID::SEED_PEASHOOTER, PlantType::PEASHOOTER, 50, 1, 240);
  auto repeater_seed = std::make_shared<Seed> (
      ImageID::SEED_REPEATER, PlantType::REPEATER, 50, 3, 240);
  ...
  auto cherryBomb_seed = std::make_shared<Seed> (
      ImageID::SEED_CHERRY_BOMB, PlantType::CHERRYBOMB, 50, 4, 240);
...
GameWorld::Update ()
{
  ...
  for (auto it = m_GameObjects.begin (); it != m_GameObjects.end ();) {
    if ((*it)->IsDead ()) it = m_GameObjects.erase (it);
    else it++;
  }
