...
static void
keyboardDownEventCallback (unsigned char key, int x, int y)
{
  GameManager::Instance ().KeyDownEvent (key, x, y);
}

static void
keyboardUpEventCallback (unsigned char key, int x, int y)
{
  GameManager::Instance ().KeyUpEvent (key, x, y);
}

static void
specialKeyboardDownEventCallback (int key, int x, int y)
{
  GameManager::Instance ().SpecialKeyDownEvent (key, x, y);
}

static void
specialKeyboardUpEventCallback (int key, int x, int y)
{
  GameManager::Instance ().SpecialKeyUpEvent (key, x, y);
}

static void
mouseButtonEventCallback (int button, int state, int x, int y)
{
  y = WINDOW_HEIGHT - y;
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) { GameManager::Instance ().MouseDownEvent (x, y); }
}
...
void
displayText (double x, double y, double z, const char *str, bool centering, void *font = GLUT_BITMAP_HELVETICA_10)
{
  if (centering) {
...
{
  if (m_pause) return;
  if (GetKey (KeyCode::QUIT)) { exit (EXIT_SUCCESS); }
  switch (m_gameState) {
  ...
  case GameManager::GameState::GAMEOVER:
    if (GetKey (KeyCode::ENTER)) { exit (EXIT_SUCCESS); }
    break;
  default:
    break;
  }
...
void
GameManager::KeyDownEvent (unsigned char key, int x, int y)
{
  KeyCode keyCode = ToKeyCode (key);
  if (keyCode != KeyCode::NONE) {
    if (m_pressedKeys.find (keyCode) == m_pressedKeys.end ()) { m_pressedKeys.insert ({ keyCode, true }); }
  }
}

void
GameManager::KeyUpEvent (unsigned char key, int x, int y)
{
  KeyCode keyCode = ToKeyCode (key);
  if (keyCode != KeyCode::NONE) {
    if (m_pressedKeys.find (keyCode) != m_pressedKeys.end ()) { m_pressedKeys.erase (keyCode); }
  }
}

void
GameManager::SpecialKeyDownEvent (int key, int x, int y)
{
...
void
GameManager::SpecialKeyUpEvent (int key, int x, int y)
{
  KeyCode keyCode = SpecialToKeyCode (key);
  if (keyCode != KeyCode::NONE) {
    if (m_pressedKeys.find (keyCode) != m_pressedKeys.end ()) { m_pressedKeys.erase (keyCode); }
...
void
GameManager::MouseDownEvent (int x, int y)
{
  ObjectBase::ClickAt (x, y);
}

bool
GameManager::GetKey (KeyCode key) const
{
  return m_pressedKeys.find (key) != m_pressedKeys.end ();
}

bool
GameManager::GetKeyDown (KeyCode key)
{
...
GameManager::Display ()
{
  ...
  TextBase::DisplayAllTexts ([=] (int x, int y, const std::string &text, double r, double g, double b, bool centering) {
    glPushAttrib (GL_CURRENT_BIT);
    glColor3f (r, g, b);
    displayText (NormalizeCoord (x, WINDOW_WIDTH),
		 NormalizeCoord (y, WINDOW_HEIGHT),
		 0,
		 text.c_str (),
		 centering,
...
double
GameManager::NormalizeCoord (double pixels, double totalPixels) const
{
...
// 将给定的归一化坐标转换为像素坐标
int
GameManager::DenormalizeCoord (double normalizedCoord, double totalPixels) const
{
...
// 绕原点顺时针旋转degree度
inline void
GameManager::Rotate (double x, double y, double degrees, double &xout, double &yout) const
{
...
std::size_t
GameManager::DrawOneObject (ImageID imageID, AnimID animID, double x, double y, std::size_t frame) const
{
  SpriteInfo spriteInfo = SpriteManager::Instance ().GetSpriteInfo (imageID, animID);
  if (spriteInfo.texture == 0) { return 0; }
  glPushMatrix ();
  ...
  if (imageID == ImageID::POLE_VAULTING_ZOMBIE) { y += 20; }
...
void
GameManager::ShowZombiesWon () const
{
  ...
  displayText (NormalizeCoord (WINDOW_WIDTH / 2, WINDOW_WIDTH),
	       NormalizeCoord (50, WINDOW_HEIGHT),
	       0,
	       "You survived [      ] waves! Press Enter to restart.",
	       true,
	       GLUT_BITMAP_HELVETICA_18);
  glPopAttrib ();
...
// 检查输入, 并获取按下了什么按键
inline KeyCode
GameManager::ToKeyCode (unsigned char key) const
{
  switch (key) {
  case '\x1B':
    return KeyCode::QUIT;
...
inline KeyCode
GameManager::SpecialToKeyCode (int key) const
{
  switch (key) {
  default:
    return KeyCode::NONE;
